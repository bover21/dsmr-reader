...
from dsmr_googledrive.googledrive.drive_api import CredentialsError, UploadError
...
class TestServices(InterceptStdoutMixin, TestCase):
    ...
    @mock.patch('dsmr_googledrive.googledrive.drive_api.Credentials')
    ...
    @mock.patch('dsmr_googledrive.services.make_request')
    ...
    def test_sync_init_setup(self, now_mock, make_request_mock):
        ...
        GoogleDriveSettings.objects.all().update(client_id="FAKE", client_secret="FAKE",
                                                 state=settings.DSMRREADER_GDRIVE_MAKE_ACCESS_REQUEST, next_sync=None)

        self.assertFalse(make_request_mock.called)
        dsmr_googledrive.services.sync()
        self.assertTrue(make_request_mock.called)

    @mock.patch('dsmr_googledrive.services.poll_server')
    @mock.patch('django.utils.timezone.now')
    def test_sync_init_polling(self, now_mock, poll_server_mock):
        now_mock.return_value = timezone.make_aware(timezone.datetime(2016, 1, 1))

        GoogleDriveSettings.objects.all().update(client_id="FAKE", client_secret="FAKE",
                                                 state=settings.DSMRREADER_GDRIVE_POLL_SERVER, next_sync=None)
        self.assertFalse(poll_server_mock.called)
        dsmr_googledrive.services.sync()
        self.assertTrue(poll_server_mock.called)

    @mock.patch('dsmr_googledrive.services.setup_directory')
    @mock.patch('django.utils.timezone.now')
    def test_sync_init_directory_setup(self, now_mock, setup_directory_mock):
        now_mock.return_value = timezone.make_aware(timezone.datetime(2016, 1, 1))

        GoogleDriveSettings.objects.all().update(client_id="FAKE", client_secret="FAKE",
                                                 state=settings.DSMRREADER_GDRIVE_SETUP_FOLDERS, next_sync=None)
        self.assertFalse(setup_directory_mock.called)
        dsmr_googledrive.services.sync()
        self.assertTrue(setup_directory_mock.called)

    @mock.patch('dsmr_googledrive.services.gdrive_sync')
    @mock.patch('django.utils.timezone.now')
    def test_sync_gdrive_sync(self, now_mock, gdrive_sync_mock):
        now_mock.return_value = timezone.make_aware(timezone.datetime(2016, 1, 1))

        GoogleDriveSettings.objects.all().update(client_id="FAKE", client_secret="FAKE",
                                                 state=settings.DSMRREADER_GDRIVE_SYNC_FILES, next_sync=None)
        self.assertFalse(gdrive_sync_mock.called)
        dsmr_googledrive.services.sync()
        self.assertTrue(gdrive_sync_mock.called)

    @mock.patch('dsmr_googledrive.services.sync_file')
    @mock.patch('dsmr_googledrive.services.should_sync_file', return_value=True)
    @mock.patch('django.utils.timezone.now')
    ...
    @mock.patch('dsmr_googledrive.services.sync_file')
    @mock.patch('dsmr_googledrive.services.should_sync_file', return_value=False)
    @mock.patch('django.utils.timezone.now')
    ...
    def test_calculate_content_hash(self):
        result = dsmr_googledrive.services.calculate_content_hash(
    ...
    def test_should_sync_file(self, stat_mock, time_mock):
        ...
        self.assertFalse(dsmr_googledrive.services.should_sync_file(FILE))
        # Skip stale file.
        stat_result = mock.MagicMock()
        ...
        self.assertTrue(dsmr_googledrive.services.should_sync_file(FILE))
    ...
    def test_reset_state(self):
        GoogleDriveSettings.objects.all().update(client_id=None, client_secret="FAKE",
                                                 state=settings.DSMRREADER_GDRIVE_SETUP_FOLDERS, next_sync=None)
        self.assertEqual(2, GoogleDriveSettings.get_solo().state)
        dsmr_googledrive.services.sync()
        self.assertEqual(0, GoogleDriveSettings.get_solo().state)

        GoogleDriveSettings.objects.all().update(client_id="FAKE", client_secret=None,
                                                 state=settings.DSMRREADER_GDRIVE_SETUP_FOLDERS, next_sync=None)
        self.assertEqual(2, GoogleDriveSettings.get_solo().state)
        dsmr_googledrive.services.sync()
        self.assertEqual(0, GoogleDriveSettings.get_solo().state)
    ...
    @mock.patch('dsmr_googledrive.googledrive.drive_api.DriveService.upload_file')
    @mock.patch('dsmr_googledrive.services.calculate_content_hash')
    @mock.patch('dsmr_googledrive.googledrive.drive_api.DriveService.init_resumable_upload_session')
    @mock.patch('dsmr_googledrive.googledrive.drive_api.DriveService.init_resumable_upload_session_existing_file')
    @mock.patch('dsmr_backup.services.backup.get_backup_directory')
    @mock.patch('dsmr_googledrive.googledrive.drive_api.DriveService.get_file_meta')
    @mock.patch('django.utils.timezone.now')
